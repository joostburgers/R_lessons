...
Note that the extraction above is purely theoretical. Traditional NER libraries still have a hard time make distinctions between entities, especially with fiction. There are more advanced techniques like BERT, are currently no easy-to-use implementations in R. In fact, the best library for this is actually CleanNLP, which uses Python's Spacy library. Unfortunately, there are currently issues with this library and it's R implementation. We are using a more rudimentary library called OpenNLP that is slower and less precise. It is a a taste of things that might be possible with NER.
...
Now load the NLP packages and the Tidy low-calorie special sauce.
...
Now that we have two separate tables we can rejoin them. Still, we can't just join them by "word" that won't work. How can we go about rejoining these two tables? We can place the *entities* in *sentences* based on whether the word is detected in the sentence. This function takes a long time depending on how many entities you have. It is best to go through the `pre_join_entities.csv` and delete all of the nonsense words and false positives.
